...
void PinyinEditor::GetRawText(char **text, int *len) {
  ...
  *text = (char *)malloc(*len);
  memcpy(*text, pinyin_table_.data(), *len);
}
...
void PinyinEditor::GetDynamicPhrase(std::list<const PhraseDatum *> *list) {
  ...
  dynamic_phrase.GetDynamicPhrase(pinyin_table_.data(), &phrase_datum_list);
...
void PinyinEditor::CreateCharsProxy() {
  ...
  char *pinyin1 = AmendPinyinString(pinyin_table_.data(), mend_pair_table);
...
char *PinyinEditor::AmendPinyinString(
  ...
  return strdup(mend_string.data());
}
